...
function insertGHData(data) {
    document.getElementById("description").innerHTML +=
        `<p>I currently work at ${data[0].company}, Inc. as a <strong>Level 2 IT Support Agent</strong></p>
        </p><br></b><p class="my-projects">Some projects I coded</p>`;

    data[1].forEach(element => {
        document.getElementById("gh").innerHTML +=
            `<a href=${element.html_url} target="_blank"><div class="project"><p class="project-name"><span>Name</span>${element.name}</p><p class="project-description"><span>Description</span>${element.description}</p><p class="project-technologies-used"><span>Technologies used</span>${element.topics}</p></div></a>`;
    });
}